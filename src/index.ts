...
import { generate, type OAuthTokenInfo } from "./generator.js";
...
async function updateAgentConfig(
  ...
  if (!agent.configFile || !existsSync(agent.configFile)) return;

  let config: Record<string, unknown> = {};
  try {
    config = JSON.parse(await readFile(agent.configFile, "utf8")) as Record<
      string,
      unknown
    >;
  } catch {
  ...
  if (agent.id === "openclaw") {
    ...
    if (!extraDirs.includes(agent.skillsDir)) {
      extraDirs.push(agent.skillsDir);
...
function parseArgs(argv: string[]): {
  ...
  if (!url) {
    console.error(
      'Usage: mcptoskill <mcp-server-url> [--name=<skill-name>] [--out=<output-dir>] [--target=<agent-id>] [--header "Key: Value"] [--skill-key=<key>]',
    );
    console.error("");
    console.error("Examples:");
    console.error("  mcptoskill https://mcp.context7.com/mcp");
    console.error(
      "  mcptoskill https://mcp.notion.com/mcp          # prompts for auth & target",
    );
    console.error(
      "  mcptoskill https://mcp.context7.com/mcp --name=context7 --out=./skills",
    );
    console.error(
      '  mcptoskill https://mcp.supabase.com/mcp?project_ref=XXX --header "Authorization: Bearer sbp_xxx"',
    );
    console.error("");
    console.error(
      "Run without --header for interactive prompts: auth (OAuth/API key/none) and target (OpenClaw/Hermes/Claude/All).",
    );
    process.exit(1);
  }
...
async function saveTokenFile(
  ...
  await mkdir(agent.tokensDir, { recursive: true });

  const tokenFilePath = join(agent.tokensDir, `${skillName}.json`);
...
function getTargets(
  ...
  const agent = getAgentById(agentsConfig, choice);
  return agent
    ? [agent]
    : [getAgentById(agentsConfig, agentsConfig.defaultAgent || "openclaw")!];
}

function getTokenPathForScript(skillName: string, agent: AgentConfig): string {
  return `${agent.tokensDir}/${skillName}.json`;
}
...
async function main() {
  ...
  if (skillKey) {
    console.warn(
      "--skill-key is deprecated. Use local OAuth instead (no flag needed).",
    );
    ...
    try {
      ...
      if (!res.ok) {
        console.error(
          "Error: skill key not found — visit mcptoskill.com to reconnect",
        );
        process.exit(1);
      }
    ...
    if (authChoice === "oauth" && provider) {
      const oauthSkillName = name ?? `mcp-${provider.id}`;
  ...
  const targetChoice = targetFromArgs
    ? targetFromArgs
    : await promptTarget(agentsConfig);
  const targets = getTargets(targetChoice, agentsConfig);
  ...
  console.log(
    `Found: ${result.serverInfo.name} v${result.serverInfo.version} — ${result.tools.length} tool(s)`,
  );
  for (const t of result.tools) {
    console.log(
      `  • ${t.name}: ${t.description.slice(0, 80)}${t.description.length > 80 ? "…" : ""}`,
    );
  }
  ...
  const slug = result.serverInfo.name
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-|-$/g, "");
  const skillNameOverride =
    oauthData && provider
      ? (name ??
        (provider.id.startsWith("mcp-") ? provider.id : `mcp-${provider.id}`))
      : undefined;
  const skillName =
    skillNameOverride ??
    (name ? (name.startsWith("mcp-") ? name : `mcp-${name}`) : undefined) ??
    (knownNames[host] ? `mcp-${knownNames[host]}` : undefined) ??
    (host.endsWith(".exa.ai") ? "mcp-exa" : `mcp-${slug}`);
  ...
  if (customOutDir) {
    ...
    const oauthTokenInfo: OAuthTokenInfo | undefined = oauthData
      ? {
          ...
          tokenFilePath: getTokenPathForScript(skillName, agent),
    ...
    const {
      ...
      skillMd,
      shellScript,
    } = generate(result, agent, skillName, oauthTokenInfo);
  ...
  for (const agent of targets) {
    ...
    if (tokenFilePath) console.log(`✓ Token saved: ${tokenFilePath}`);
    console.log(`✓ Skill installed: ${finalSkillName} (${agent.name})`);
    console.log(`  ${skillDir}/`);

    if (agent.id === "openclaw") {
      await updateAgentConfig(agent, finalSkillName);
    }
  }

  const localScriptsDir = join(process.cwd(), "scripts");
  await mkdir(localScriptsDir, { recursive: true });
  const lastAgent = targets[targets.length - 1];
  const lastOutDir = lastAgent.skillsDir;
  const finalSkillName = skillNameOverride ?? skillName;
  const localScriptPath = join(localScriptsDir, `${finalSkillName}.sh`);
  await copyFile(
    join(lastOutDir, finalSkillName, "scripts", `${finalSkillName}.sh`),
    localScriptPath,
  );
  ...
  console.log(
    `  "use ${result.serverInfo.name} to ${result.tools[0]?.name?.replace(/-/g, " ") ?? "run a tool"}"`,
  );
  console.log("");
  console.log("Test the script directly:");
  console.log(
    `  ${localScriptPath} ${result.tools[0]?.name ?? "tool-name"} '{}'`,
  );
}
